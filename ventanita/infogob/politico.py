...
def get_dic_links(soup_ficha):
    ...
    tablas_enlaces = soup_ficha.find(id="generales-enlace").find_all("table")
    links = [a.attrs.get("href") for a in [tabla.find("a") for tabla in tablas_enlaces]]
    assert len(links)==3, "Hay {num} links, deberian ser 3".format(num=len(links))
    return {"hoja_vida": links[0],
            "plan_gobierno": links[1],
            "declaracion_bienes": links[2]
    }
